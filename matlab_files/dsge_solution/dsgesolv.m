...
%                            (parameters that are derived from the values
%                            of other parameters).
...
function [TTT,RRR,CCC,valid] = dsgesolv(para,mspec,nant,varargin)
...
if mspec == 2 || mspec == 3,
    if (rstar < 1)
        ...
        disp(gam)
        disp(bet)
        TTT = zeros(nstate,nstate);
        RRR = zeros(nstate,2);
        valid = - 3;
        return;
    elseif ( cstar <= 0 ) || ( ystar <= 0 ) || (istar <= 0 )
        nstate = 1;
        disp('cstar or ystar or istar < 0 ');
        TTT = zeros(nstate,nstate);
        RRR = zeros(nstate,2);
        valid = - 3;
        return;
...
if length(varargin) == 1
    ...
    alt_policy = varargin{1};
elseif length(varargin) == 2
  % pass in final state vector
    alt_policy = varargin{1};
    zend = varargin{2};
else
    alt_policy = 0;
...
if any(mspec==[805 904]),
    ...
    % Note added 8/20/2014, MDC: We construct state for expected inflation
    % using fact
    %
